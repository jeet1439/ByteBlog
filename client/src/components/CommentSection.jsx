    ...
    <div className="max-w-2xl mx-auto w-full p-3">
      {currentUser ? (
        <div className="flex items-center gap-1 my-3 text-gray-500 text-sm">
            <p>Signed in as: </p>
            <img className="h-7 w-7 object-cover rounded-full" src={currentUser.profilePic.url} alt="" />
            <Link to={'/dashboard?tab=profile'} className="text-cyan-600 hover:underline">
            @ {currentUser.username}
            </Link>
        </div>
      ) : (
        <div className="text-sm text-cyan-600 my-3 text-center ">
      ...
      {currentUser && (
        <form onSubmit={handleSubmit}className="border border-teal-500 rounded-md p-3">
            ...
            <div className="flex justify-between items-center mt-5">
                ...
                <Button outline gradientDuoTone="greenToBlue" type='submit'>Sublit</Button>
            </div>
